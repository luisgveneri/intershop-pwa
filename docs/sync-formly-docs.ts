...
const folder = 'src/app/shared/formly/reusable-configurations';

// extract all provided configurations from module and map to corresponding files
const moduleContent = readFileSync(`${folder}/reusable-configurations.module.ts`, {
  encoding: 'utf-8',
});

const configs = moduleContent.match(
  /(?<={ provide: REUSABLE_FORMLY_FIELD_CONFIGURATION, useClass: )(.+)(?=Configuration, multi: true })/gm
);
